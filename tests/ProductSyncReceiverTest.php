...
final class ProductSyncReceiverTest extends TestCase {
	public function test_current_owner_projection_reads_only_committed_products_and_checks_binding(): void {
		...
		$input = array( 'id' => 'projection-test', 'dataset' => 'kala.db', 'revision' => 'sha256:input' );
		$source = array_merge( $input, array( 'revision' => 'sha256:final' ) );
		$key = hash( 'sha256', "projection-test\nkala.db" );
		$state = array( 'sources' => array( $key => array(
			'input_mode' => 'patris_inputs', 'input_source' => $input, 'source' => $source,
			'products' => array(
				array( 'product_code' => 'unpriced', 'record_hash' => 'sha256:blank', 'total_stock' => '7', 'private_internal' => 'must-not-leak' ),
				array( 'product_code' => 'priced', 'record_hash' => 'sha256:priced', 'final_price' => '100', 'pricing_catalog_revision' => $catalog['revision'] ),
			),
		) ) );
		$GLOBALS['digitalogic_test_options'][ Digitalogic_Product_Sync_Receiver::STATE_OPTION ] = $state;
		$request = array( 'source_id' => $input['id'], 'source_dataset' => $input['dataset'], 'source_revision' => $input['revision'], 'owner_catalog_revision' => $catalog['revision'] );
	...
	public function test_unpriced_positive_stock_legacy_leaf_is_repaired_without_a_duplicate_or_hidden_save(): void {
		$product                = array(
			'product_code' => '102006010',
			'name'         => 'Positive stock without canonical price',
			'total_stock'  => 4,
			'weight_grams' => 100,
		...
		$stopped = $receiver->reconcile( 'tests', 'ALLANBAR', 1 );
		$this->assertNotInstanceOf( WP_Error::class, $stopped );
		$this->assertSame( 1, $stopped['materialization_mismatch_stopped'] );
		$this->assertSame( 1, $stopped['pending_products'] );
		$this->assertCount( $seed_saves, $GLOBALS['digitalogic_test_wc_product_saves'] );

		$written = Digitalogic_Patris_Feed::instance()->apply_product_feed( wc_get_product( $id ), $product );
		...
		$this->assertSame( 0, $repaired['materialization_queued'] );
	...
	public function test_reconciliation_backfills_stale_materialization_projection_idempotently(): void {
		...
		$receiver = Digitalogic_Product_Sync_Receiver::instance();
		$seed     = $receiver->receive( $this->snapshot( $products, array(), true ) );
		$this->assertNotInstanceOf(
			WP_Error::class,
			$seed,
			$seed instanceof WP_Error
				? $seed->get_error_code() . ': ' . $seed->get_error_message() . ' ' . wp_json_encode( $seed->get_error_data() )
				: ''
		);
		...
		$GLOBALS['digitalogic_test_wc_products']     = array();
		$GLOBALS['digitalogic_test_post_meta_cache'] = array();
		$seed_save_count                             = count( $GLOBALS['digitalogic_test_wc_product_saves'] );
		...
		$this->assertSame(
			array(
				...
				'after_code'      => 'LEGACY-META-02',
			),
			$state['materialization_scan_cursor']
		);
		$this->assertSame( 2, $GLOBALS['wpdb']->identifier_query_count );
		...
		$this->assertNotInstanceOf( WP_Error::class, $repaired );
		$this->assertSame( 0, $repaired['materialization_queued'] );
		$this->assertSame( 1, $repaired['sources'][0]['woocommerce']['attempted'] );
		$this->assertSame( 1, $repaired['materialization_metadata_backfilled'] );
		$this->assertSame( 0, $repaired['materialization_mismatch_stopped'] );
		$this->assertSame( 0, $repaired['pending_products'] );
		$this->assertCount( $seed_save_count, $GLOBALS['digitalogic_test_wc_product_saves'] );
