...
final class ShippingMethodServiceTest extends TestCase {
    ...
    public function test_canonical_shipping_fields_require_explicit_supported_currency(): void {
        ...
        $this->assertSame('IRR', $created['currency']);
    ...
    public function test_assignment_writes_one_meta_key_and_batch_is_sparse(): void {
        ...
        $this->assertSame('85', $assigned['shipping_price_per_kg']);
        $this->assertSame('CNY', $assigned['shipping_price_per_kg_currency']);
    ...
    public function test_lock_failure_is_retryable_and_write_free(): void {
        ...
        $this->assertSame($releaseCount, $GLOBALS['wpdb']->release_count);
    }
    ...
    public function test_invalid_currency_is_omitted_and_input_ranges_are_strict(): void {
        ...
        $overlap = $this->service->create_method(array(
            ...
            'tiered_rates' => array(
                ...
                array('min_weight_kg' => 4, 'max_weight_kg' => 8, 'price_per_kg' => 2700000),
            ),
        ));
        ...
        $tiered = $this->service->create_method(array(
            ...
            'tiered_rates' => array(
                array('min_weight_kg' => 0, 'max_weight_kg' => 5, 'price_per_kg' => 3000000),
                array('min_weight_kg' => 6, 'max_weight_kg' => null, 'price_per_kg' => 2700000),
            ),
        ));
        $this->assertNotInstanceOf(WP_Error::class, $tiered);
        $this->assertSame('IRR', $tiered['currency']);
        $this->assertSame('3000000', $tiered['tiered_rates'][0]['price_per_kg']);
        $this->assertArrayNotHasKey('currency', $tiered['tiered_rates'][0]);
    }
	...
	public function test_method_decimals_are_bounded_exact_strings_in_every_projection(): void {
		...
		$this->assertNotInstanceOf(WP_Error::class, $created);
		$this->assertSame('123456789.123456789125', $created['price_per_kg']);
		$this->assertSame('0.000000000005', $created['minimum_charge']);
		$this->assertSame('5000.000000000005', $created['volumetric_divisor_cm3_per_kg']);
		$this->assertSame('0.000000000005', $created['tiered_rates'][0]['min_weight_kg']);
		$this->assertSame('1.000000000005', $created['tiered_rates'][0]['max_weight_kg']);
		...
		$this->assertSame('123456789.123456789125', $method['price_per_kg']);
		$this->assertSame('0.000000000005', $method['minimum_charge']);
		$this->assertSame('9.876543210125', $method['tiered_rates'][0]['price_per_kg']);
		$json = json_encode($method, JSON_THROW_ON_ERROR);
		$this->assertStringContainsString('123456789.123456789125', $json);
    ...
    public function test_installed_method_option_is_migrated_without_losing_rates(): void {
		...
		$GLOBALS['digitalogic_test_option_cache'][Digitalogic_Shipping_Method_Service::METHODS_OPTION] = array(
			'stale' => array('id' => 'stale'),
		);
        $this->resetSingleton(Digitalogic_Shipping_Method_Service::class);
        ...
        $this->assertSame('77', $method['price_per_kg']);
        $this->assertSame('5', $method['minimum_charge']);
        $this->assertSame('70', $method['tiered_rates'][0]['price_per_kg']);
        $this->assertArrayNotHasKey($retiredRateKey, $method);
        $this->assertArrayNotHasKey($retiredMinimumKey, $method);
        $this->assertSame(
            'complete',
            $GLOBALS['digitalogic_test_options']['digitalogic_shipping_currency_migration_complete']
        );
		$this->assertArrayNotHasKey(Digitalogic_Shipping_Method_Service::METHODS_OPTION, $GLOBALS['digitalogic_test_option_cache']);
		$this->assertContains(
			array(Digitalogic_Shipping_Method_Service::METHODS_OPTION, 'options'),
			$GLOBALS['digitalogic_test_cache_deletes']
		);
    }

	public function test_migration_marker_is_not_persisted_when_method_write_cannot_be_verified(): void {
		unset($GLOBALS['digitalogic_test_options']['digitalogic_shipping_currency_migration_complete']);
		...
		$this->assertArrayNotHasKey('digitalogic_shipping_currency_migration_complete', $GLOBALS['digitalogic_test_options']);
		$this->assertArrayHasKey(
			'shipping_price_per_kg_cny',
			$GLOBALS['digitalogic_test_options'][Digitalogic_Shipping_Method_Service::METHODS_OPTION]['legacy']
		);
	}
