...
class Digitalogic_Pricing_Apply_Jobs {
	...
	private const MAX_REQUESTS              = 256;
	...
	public function admit( $settings, $expected_revision, $request_id, $binding, $metadata, $request_context = array() ) {
		...
		$scope = $this->normalized_scope( $this->read_pricing_scope() );
	...
	public function cancel( $identifier, $source ) {
		...
		$status = $this->with_registry_lock(
			function ( $registry ) use ( $identifier, $source ) {
				...
				if (
					...
					&& (int) $job['lease_expires_at'] <= $this->now()
		...
		if ( ! $this->schedule_worker( (string) $status['job_id'], 1 ) ) {
			$recorded = $this->record_schedule_failure( (string) $status['job_id'] );
	...
	public function run_job( $job_id ) {
		...
		if ( ! $this->schedule_worker( (string) $job_id, 1 ) ) {
			$this->record_schedule_failure( (string) $job_id );
		}
	}
	...
	public function recover_unarmed_admissions() {
		...
		foreach ( (array) $candidates['deliveries'] as $job_id ) {
			if ( ! $this->schedule_worker( $job_id, 1 ) ) {
				$this->record_schedule_failure( $job_id );
			}
		}
	}
	...
	public function watchdog( $job_id ) {
			...
			function ( $registry ) use ( $job_id, $probe ) {
				...
				if ( is_wp_error( $probe ) || true === $probe ) {
					...
					if ( (int) $job['watchdog_failures'] >= self::MAX_WATCHDOG_FAILURES || $now >= (int) $job['recovery_deadline_at'] ) {
						...
						$job                    = $this->terminal_job( $job, 'outcome_unknown', 'readback_required', $job['last_error'] );
					}
				} else {
		...
		if ( ! empty( $state['retry'] ) && ! $this->schedule_worker( $job_id, 1 ) ) {
			$this->record_schedule_failure( $job_id );
		}
		$this->reschedule_watchdog( $job_id );
	}
	...
	private function apply_phase_error( $job, $error ) {
		...
		if ( is_array( $data ) && ! empty( $data['readback_required'] ) ) {
			$job['outbox']['state'] = 'failed';
			return $this->terminal_job( $job, 'outcome_unknown', 'readback_required', $job['last_error'] );
		}
		...
		if ( 'publishing' === (string) $job['phase'] ) {
			...
			$job['outbox']['state'] = 'failed';
			return $this->terminal_job( $job, 'outcome_unknown', 'event_delivery_readback_required', $job['last_error'] );
		}
	...
	private function existing_request( $registry, $request ) {
		...
		if ( ! hash_equals( (string) ( $record['request_fingerprint'] ?? '' ), (string) $request['request_fingerprint'] ) ) {
		...
		$job = $registry['jobs'][ $record['job_id'] ] ?? null;
		if ( ! is_array( $job ) ) {
	...
	private function arm_admission( $job_id ) {
		...
		if ( ! $watchdog_scheduled || ! $worker_scheduled ) {
			$this->unschedule_job( $job_id );
			return $this->terminalize_scheduler_admission_failure( $job_id );
		}

		return $this->with_registry_lock(
			function ( $registry ) use ( $job_id, $claim ) {
				$job = $registry['jobs'][ $job_id ] ?? null;
				if ( ! is_array( $job ) ) {
					return $this->error( 'digitalogic_pricing_apply_job_not_found', 'Pricing apply job was not found.', 404 );
				}
	...
	private function terminalize_scheduler_admission_failure( $job_id ) {
		return $this->with_registry_lock(
			function ( $registry ) use ( $job_id ) {
				$job = $registry['jobs'][ $job_id ] ?? null;
				if ( ! is_array( $job ) ) {
				...
				$pristine                 = ! $this->job_has_effects( $job ) && empty( $job['phase_resolution_required'] ) && 'settings' === (string) $job['phase'];
				if ( $pristine ) {
					$job['outbox']['state'] = 'failed';
					...
					$job['outbox']['state'] = 'failed';
					$job                    = $this->terminal_job( $job, 'outcome_unknown', 'readback_required', $error );
					$accepted               = true;
				} else {
	...
	/** Record a failed continuation while init/watchdog recovery remains finite. */
	private function record_schedule_failure( $job_id, $kind = 'worker' ) {
		$kind   = 'watchdog' === $kind ? 'watchdog' : 'worker';
		$status = $this->with_registry_lock(
			function ( $registry ) use ( $job_id, $kind ) {
				$job = $registry['jobs'][ $job_id ] ?? null;
				if ( ! is_array( $job ) || $this->is_terminal( $job ) ) {
					return null;
				}
				...
				if ( (int) $job['schedule_failures'] >= self::MAX_SCHEDULE_FAILURES || $this->now() >= (int) $job['recovery_deadline_at'] ) {
					$unknown                = $this->job_has_effects( $job ) || ! empty( $job['phase_resolution_required'] ) || 'publishing' === (string) $job['phase'];
					$job['outbox']['state'] = 'failed';
					$job                    = $this->terminal_job( $job, $unknown ? 'outcome_unknown' : 'failed', $unknown ? 'readback_required' : 'schedule_failed', $error );
				} else {
					if ( 'watchdog' === $kind ) {
						$job['schedule_state']            = 'unarmed';
						$job['schedule_token']            = '';
	...
	private function reschedule_watchdog( $job_id ) {
		if ( ! $this->schedule_watchdog( $job_id, self::HEARTBEAT_GRACE_SECONDS ) ) {
			$this->record_schedule_failure( $job_id, 'watchdog' );
		}
	}
	...
	private function save_registry( $registry ) {
		...
		foreach ( $registry['requests'] as $request_id => $request ) {
			if ( ! is_array( $request ) || ! isset( $registry['jobs'][ $request['job_id'] ] ) ) {
				unset( $registry['requests'][ $request_id ] );
			}
		}
		if ( count( $registry['requests'] ) > self::MAX_REQUESTS ) {
			$registry['requests'] = array_slice( $registry['requests'], -self::MAX_REQUESTS, null, true );
		}
	...
	protected function read_pricing_scope() {
		return Digitalogic_Excel_Pricing_Sync::instance()->pricing_apply_scope();
	}
