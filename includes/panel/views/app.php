        ...
        <div class="dlp-layout" v-cloak>
            ...
            <main class="dlp-main">
                ...
                <div v-if="currencyJob && currencyJob.message_fa" class="dlp-job-status" :class="'is-' + currencyJob.status" role="status" aria-live="polite">
					...
					<strong v-if="currencyJob.job_id && currencyJob.generation && typeof currencyJob.progress === 'number'">{{ formatNumber(currencyJob.progress) }}%</strong>
                </div>
                                ...
                                <tbody>
                                    <tr v-for="product in filteredProducts" :key="product.id" :data-product-row="product.id" :class="{'is-edited': rowEdited(product), 'is-selected': isProductSelected(product), 'is-active-product': selectedProduct && selectedProduct.id === product.id}" @click="selectProductRow(product)" @dblclick="openProductPanel(product, {reveal: true})" @focusin="selectProductRow(product)" @contextmenu.prevent="openProductRowContext(product, $event)">
                                        ...
                                        <td v-for="(column, columnIndex) in visibleProductColumns" :key="column.key" :class="[cellClass(column), {'is-sticky-first-data-column': columnIndex === 0, 'is-resizing': resizingColumn === column.key, 'is-warehouse-column': column.warehouse}]" :data-column-key="column.key" :data-column-priority="column.priority || 1">
											<button v-if="column.type === 'select'" class="dlp-editable-cell dlp-select-cell" :class="{'is-readonly': !isProductColumnEditable(product, column) || !productEditMode}" :disabled="!isProductColumnEditable(product, column) || !productEditMode" :title="productColumnEditReason(product, column)" tabindex="0" :data-grid-row="product.id" :data-grid-col="column.key" @click.stop="openSelectCell('product', product, column, $event)" @focus="selectProductRow(product)" @keydown="onGridCellKeydown($event, product, column)">
											...
											<span v-else-if="!isCellEditing('product', product, column)" class="dlp-editable-cell" :class="{'is-readonly': !isProductColumnEditable(product, column) || !productEditMode}" :title="productColumnEditReason(product, column)" tabindex="0" :data-grid-row="product.id" :data-grid-col="column.key" @pointerdown.stop="isProductColumnEditable(product, column) && productEditMode && startCellEdit('product', product, column, $event)" @focus="selectProductRow(product)" @keydown="onGridCellKeydown($event, product, column)">
											...
											<template v-else-if="column.field === 'patris_product_code'"><span>{{ formatColumnValue(product, column) }}</span><button v-if="productCodePendingProposal(product)" type="button" class="dlp-button dlp-product-code-retry" @pointerdown.stop @keydown.stop @click.stop="retryPendingProductCode(product)"><span>{{ t.productCodeRetryPending }}</span> <code dir="ltr">{{ productCodePendingProposal(product) }}</code></button></template>
                                ...
                                <table class="dlp-table dlp-report-table" :aria-label="reportView === 'price_list' ? t.priceList : t.problemRows">
                                    ...
                                    <tbody>
                                        <tr v-for="(item, itemIndex) in reports.rows" :key="item.status + ':' + (item.woo_id || item.product_code) + ':' + itemIndex">
                                            ...
                                            <td class="dlp-cell-numeric"><bdi dir="auto">{{ reportSparseValue(item.source, 'partner_price_source') }}</bdi></td>
