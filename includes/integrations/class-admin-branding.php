...
final class Digitalogic_Plugin_Admin_Branding {
    ...
    private const DIGITS_SECURE_SCRIPT_HANDLE = 'digits-frontend-script';
    private const DIGITS_SECURE_SCRIPT_FILE = 'digits/assets/js/script.min.js';
    private const THEME_STORAGE_KEY = 'digitalogic-admin-theme';
    public const ADMIN_COLOR_LIGHT = 'digitalogic-light';
    public const ADMIN_COLOR_DARK = 'digitalogic-dark';
    ...
    public static function init(): void {
        ...
        add_action('admin_init', [self::class, 'register_admin_color_schemes']);
        ...
        add_action('wp_ajax_digitalogic_set_admin_theme', [self::class, 'ajax_set_admin_theme']);
        ...
        add_filter('login_headerurl', [self::class, 'login_header_url'], 9999);
        add_filter('login_headertext', [self::class, 'login_header_text'], 9999);
    ...
    public static function output_theme_bootstrap(): void {
        ...
        $initial_theme = wp_json_encode(self::current_user_theme());
        $admin_color = wp_json_encode(self::current_user_admin_color());
...
(function() {
    ...
    var adminColor = {$admin_color};

    if (adminColor !== 'digitalogic-light' && adminColor !== 'digitalogic-dark') {
        try {
...
HTML;
    ...
    public static function enqueue_login_assets(): void {
        self::enqueue_digits_secure_form_assets();
    ...
    public static function admin_bar_menu(WP_Admin_Bar $admin_bar): void {
        ...
        $admin_bar->add_node([
            ...
            'title' => '<span class="dg-brand-node"><span class="dashicons dashicons-admin-generic dg-brand-node__icon" aria-hidden="true"></span><span class="dg-brand-node__text">Digitalogic</span></span>',
            'href'  => home_url('/'),
            'meta'  => [
        ...
        $admin_bar->add_node([
            ...
            'title'  => '<span class="dg-adminbar-theme-toggle" data-dg-theme-toggle-button><span class="dashicons dashicons-lightbulb" aria-hidden="true"></span><span class="dg-toggle-text" data-dg-theme-toggle-label>Switch theme</span></span>',
            'href'   => '#',
            'meta'   => [
                'class' => 'digitalogic-theme-toggle',
                'title' => __('Switch admin theme', 'digitalogic'),
            ],
        ]);
    ...
    public static function register_admin_color_schemes(): void {
        $style_url = DIGITALOGIC_PLUGIN_URL . self::STYLE_FILE;

        wp_admin_css_color(
            self::ADMIN_COLOR_LIGHT,
            __('Digitalogic Light', 'digitalogic'),
            $style_url,
            ['#f6f8fb', '#ffffff', '#2563eb', '#0f172a'],
            ['base' => '#475569', 'focus' => '#2563eb', 'current' => '#0f172a']
        );

        wp_admin_css_color(
            self::ADMIN_COLOR_DARK,
            __('Digitalogic Dark', 'digitalogic'),
            $style_url,
            ['#0f172a', '#172033', '#60a5fa', '#f8fafc'],
            ['base' => '#cbd5e1', 'focus' => '#93c5fd', 'current' => '#ffffff']
        );
    }

    public static function ajax_set_admin_theme(): void {
        check_ajax_referer('digitalogic_admin_theme', 'nonce');

        if (!is_user_logged_in()) {
            wp_send_json_error(['message' => __('You must be logged in to change the admin theme.', 'digitalogic')], 401);
        }

        $theme = isset($_POST['theme']) ? sanitize_key(wp_unslash($_POST['theme'])) : '';
        $result = self::set_user_theme(get_current_user_id(), $theme);
    ...
    public static function current_user_theme(): string {
        return self::theme_from_admin_color(self::current_user_admin_color());
    }

    public static function current_user_admin_color(): string {
        if (!is_user_logged_in()) {
            return self::ADMIN_COLOR_LIGHT;
        }

        $color = get_user_option('admin_color', get_current_user_id());
        return is_string($color) && $color !== '' ? $color : self::ADMIN_COLOR_LIGHT;
    }

    public static function theme_from_admin_color(string $admin_color): string {
        if ($admin_color === self::ADMIN_COLOR_DARK || in_array($admin_color, ['midnight', 'ectoplasm', 'coffee'], true)) {
            return 'dark';
        }
    ...
    public static function admin_color_for_theme(string $theme): string {
        return $theme === 'dark' ? self::ADMIN_COLOR_DARK : self::ADMIN_COLOR_LIGHT;
    }

    public static function set_user_theme(int $user_id, string $theme) {
        if ($user_id <= 0) {
            return new WP_Error('digitalogic_theme_user_required', __('A user is required to update the admin theme.', 'digitalogic'));
        }

        if ($theme !== 'light' && $theme !== 'dark') {
            return new WP_Error('digitalogic_theme_invalid', __('Choose either light or dark mode.', 'digitalogic'));
        }

        $admin_color = self::admin_color_for_theme($theme);
        update_user_option($user_id, 'admin_color', $admin_color, true);
    ...
    private static function enqueue_digits_secure_form_assets(): void {
        $script_path = WP_PLUGIN_DIR . '/' . self::DIGITS_SECURE_SCRIPT_FILE;
        ...
        wp_enqueue_script(
            self::DIGITS_SECURE_SCRIPT_HANDLE,
            plugins_url(self::DIGITS_SECURE_SCRIPT_FILE),
        ...
        wp_add_inline_script(
            self::DIGITS_SECURE_SCRIPT_HANDLE,
            'var dig_script = window.dig_script = ' . wp_json_encode(self::digits_secure_form_config()) . ';' . "\n" . self::digits_secure_form_compat_script(),
            'before'
        );
    }

    private static function digits_secure_form_config(): array {
        return [
            'direction' => is_rtl() ? 'rtl' : 'ltr',
            'ajax_url' => admin_url('admin-ajax.php'),
            'ErrorPleasetryagainlater' => 'خطا! لطفاً دوباره تلاش کنید.',
            'fillAllDetails' => 'لطفاً اطلاعات مورد نیاز را کامل کنید.',
            'accepttac' => 'لطفاً شرایط و ضوابط را بپذیرید.',
            'dig_dsb' => '-1',
            'useStrongPasswordString' => 'لطفاً رمز عبور قوی‌تری وارد کنید.',
            'strong_pass' => '1',
            'InvalidMobileNumber' => 'شماره موبایل نامعتبر است.',
            'Invaliddetails' => 'اطلاعات وارد شده معتبر نیست.',
            'InvalidOTP' => 'کد تایید نامعتبر است.',
        ];
    }

    private static function digits_secure_form_compat_script(): string {
        return <<<'JS'
(function(window, document, $) {
    ...
    function fieldData(field, name) {
        if ($ && field && typeof field.data === 'function') {
            return field.data(name);
        }

        return field && field.dataset ? field.dataset[name] : undefined;
    }

    window.show_countrycode_field = window.show_countrycode_field || function(mobileField) {
        var value = normalizeDigits(fieldValue(mobileField)).trim();
        var dataType = fieldData(mobileField, 'type');

        if (String(dataType) === '2') {
            return true;
        }

        return value !== '' && /^[+]?\d+$/.test(value) && value.charAt(0) !== '+';
    };

    window.hideDigMessage = window.hideDigMessage || function() {
        document.querySelectorAll('.dig_popmessage, .dig_error_message').forEach(function(node) {
            node.remove();
        });
    };

    function showDigitsMessage(message, tone) {
        if (!message) {
            return;
        }

        window.hideDigMessage();

        var box = document.createElement('div');
        box.className = 'dig_popmessage dig_error_message dig_popmessage_right ' + (tone === 'success' ? 'dig_success_msg' : 'dig_critical_msg');
        box.setAttribute('role', tone === 'success' ? 'status' : 'alert');
        box.innerHTML = '<div class="dig_popmessage_contents"><div class="dig_lasele"><div class="dig_lase_message"></div></div><button type="button" class="dig_popdismiss" aria-label="Dismiss"></button></div>';
        box.querySelector('.dig_lase_message').textContent = message;
        document.body.appendChild(box);

        var dismiss = box.querySelector('.dig_popdismiss');
        if (dismiss) {
            dismiss.addEventListener('click', window.hideDigMessage);
        }
    }

    window.showDigErrorMessage = window.showDigErrorMessage || function(message) {
        showDigitsMessage(message, 'error');
    };

    window.showDigNoticeMessage = window.showDigNoticeMessage || function(message) {
        showDigitsMessage(message, 'warning');
    };

    window.showDigSuccessMessage = window.showDigSuccessMessage || function(message) {
        showDigitsMessage(message, 'success');
    };
...
JS;
    ...
    private static function client_config(bool $is_login, bool $is_frontend_auth = false): array {
        $config = [
            ...
            'initialTheme' => self::current_user_theme(),
            'adminColor' => self::current_user_admin_color(),
            ...
            'flagBaseUrl' => DIGITALOGIC_PLUGIN_URL . 'assets/images/flags/',
            'digitsRecaptchaSiteKey' => (string) get_option('digits_recaptcha_site_key', ''),
            'recaptchaScriptBase' => 'https://www.recaptcha.net/recaptcha/api.js',
            'labels' => [
                'dark' => 'تیره',
                'light' => 'روشن',
        ...
        $config['labels'] = self::client_labels();

        return $config;
    }

    private static function client_labels(): array {
        $locale = strtolower(determine_locale());
        $is_fa = str_starts_with($locale, 'fa');
        ...
        if (!$is_fa) {
            return [
                ...
                'runtimeError' => 'An unexpected page error occurred. Refresh the page and try again.',
                'requestError' => 'The background request failed. Try again or refresh the page.',
                'requiredUsername' => 'Enter your username, email, or mobile number.',
                'requiredPassword' => 'Enter your password.',
                'recaptchaUnavailable' => 'Login protection has not finished loading. Try again in a moment.',
                'usernameEmailPhone' => 'Username, email, or mobile number',
                'identityPlaceholder' => 'Username, email, or mobile number',
                'password' => 'Password',
                'showPassword' => 'Show password',
                'hidePassword' => 'Hide password',
                'phoneIdentity' => 'Mobile',
                'emailIdentity' => 'Email',
                'usernameIdentity' => 'Username',
                'identity' => 'Login',
                'applyLanguage' => 'Change',
                'language' => 'Language',
                'register' => 'Register',
        ...
        return [
            ...
            'toggleToDark' => 'تغییر به حالت تیره',
            'toggleToLight' => 'تغییر به حالت روشن',
            'runtimeError' => 'یک خطای غیرمنتظره در صفحه رخ داد. صفحه را تازه‌سازی کنید و دوباره ادامه دهید.',
            'requestError' => 'درخواست پس‌زمینه با خطا روبه‌رو شد. دوباره تلاش کنید یا صفحه را تازه‌سازی کنید.',
            'requiredUsername' => 'نام کاربری، ایمیل یا شماره موبایل را وارد کنید.',
            'requiredPassword' => 'رمز عبور را وارد کنید.',
            'recaptchaUnavailable' => 'محافظت ورود هنوز کامل بارگذاری نشده است. چند لحظه دیگر دوباره تلاش کنید.',
            'usernameEmailPhone' => 'نام کاربری، ایمیل یا شماره موبایل',
            'identityPlaceholder' => 'نام کاربری، ایمیل یا شماره موبایل',
            'password' => 'رمز عبور',
            'showPassword' => 'نمایش رمز عبور',
            'hidePassword' => 'پنهان کردن رمز عبور',
            'phoneIdentity' => 'موبایل',
            'emailIdentity' => 'ایمیل',
            'usernameIdentity' => 'نام کاربری',
            'identity' => 'ورود',
            'applyLanguage' => 'تغییر',
            'language' => 'زبان',
            'register' => 'ثبت نام',
    ...
    private static function dynamic_css(): string {
        ...
        $yekan_bakh = content_url('/uploads/2025/09/YekanBakh-Regular.woff2');
...
@font-face {
    font-family: "DigitalogicYekanBakh";
    src: url("{$yekan_bakh}") format("woff2");
    font-style: normal;
    font-weight: 400;
    font-display: swap;
}
:root {
    ...
    --dg-placeholder-font: "DigitalogicYekanBakh", "DigitalogicFanum", "IRANSans", system-ui, sans-serif;
