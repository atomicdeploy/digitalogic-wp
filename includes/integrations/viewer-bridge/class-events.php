...
final class Events
{
    ...
    public static function product_commits_complete(): void
    {
        ...
        $values = Live_State::product_event_values(array_map(
    ...
    public static function menu_changed(int $menu_id): void
    {
        ...
        self::emit(
            ...
            'catalog',
            'catalog:categories',
            Revision::hash(array('menuId' => $menu_id, 'items' => $safe_items)),
        ...
        foreach (Live_State::category_event_values() as $entity_id => $value) {
            ...
            self::emit(
                ...
                (string) (
                    $value['revision']
                    ?? Revision::hash(
                        array('entityId' => $entity_id, 'menuId' => $menu_id)
                    )
                ),
        ...
        foreach ($codes as $code) {
            ...
            if ($new_revision === '') {
                $changes[$code] = array(
                    'type' => 'source_removed',
                    'revision' => Revision::hash(
                        array('code' => $code, 'sourceRemoved' => true)
                    ),
                );
            } else {
                $changes[$code] = array(
