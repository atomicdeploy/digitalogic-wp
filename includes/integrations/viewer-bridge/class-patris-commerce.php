...
final class Patris_Commerce
{
    ...
    public static function ingest(WP_REST_Request $request)
    {
        ...
        try {
            ...
            return new WP_Error(
                ...
                array('status' => 503, 'projectionStored' => true)
            );
        }
        return array(
            'ok' => true,
    ...
    private static function normalize_customer_base($record)
    {
        ...
        return array('id' => $id, 'displayLabel' => $expected_label);
    }
    ...
    private static function emit_projection_changes(
        ...
        foreach (
            array(
                'order' => array('collection' => 'orders', 'id' => 'id'),
                'customer' => array('collection' => 'customers', 'id' => 'id'),
