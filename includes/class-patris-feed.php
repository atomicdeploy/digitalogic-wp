...
class Digitalogic_Patris_Feed {
    ...
    private function normalize_product($row) {
        ...
        return array(
            ...
            'price_source_amount' => $this->clean_number($row['price_source_amount'] ?? null),
            'price_source_currency' => strtoupper($this->clean_string($row['price_source_currency'] ?? '')),
            'price_source_kind' => $this->clean_string($row['price_source_kind'] ?? ''),
            'weight_grams' => $this->clean_number($row['weight_grams'] ?? null),
            'location' => $this->clean_string($row['location'] ?? ''),
            'price_rounding_digits' => $this->clean_number($row['price_rounding_digits'] ?? null),
            'price_rounding_mode' => $this->clean_string($row['price_rounding_mode'] ?? ''),
