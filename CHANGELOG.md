...
- Made the panel reports usable for the full result set with cached summaries, one-category-at-a-time pagination, refresh deduplication, client-language category titles, visible generation time and diagnostic details, catalog-generation invalidation that cannot republish stale in-flight data, and a server-side build lock against concurrent forced refreshes.
