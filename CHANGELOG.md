...
- An import-ready Google Apps Script with separate Products/Categories tabs, exact Patris Code key upserts, bilingual RTL/LTR headers, manual refresh, idempotent scheduled synchronization, and Script Properties-only credentials.
- Standalone REST, WP-CLI, and n8n integration guidance with explicit status/error columns and per-record/page revisions.

### Changed
- Kept Google Sheets catalog rows sparse and versionless while preserving explicit upstream null values and stable record, page, and catalog revisions.
